...
app.get('*', (req, res, next) => {
  ...
  res.sendFile(path.join(distPath, 'index.html'), (err) => {
    if (err) {
      res.status(404).send('Frontend not built yet or running on Vite (http://localhost:3000). Please visit http://localhost:3000');
    }
  });
});

app.listen(PORT, '127.0.0.1', () => {
  console.log(`🚀 Standalone Backend API running on http://127.0.0.1:${PORT}`);
});
